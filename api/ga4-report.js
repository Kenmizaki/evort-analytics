...
async function getRegionsReport(startDate, endDate, urlFilter, urlMatchType) {
  ...
  if (urlFilter) {
    ...
    request.dimensionFilter = {
      filter: { fieldName: 'pagePath', stringFilter: { value: urlFilter, matchType: gaMatchType } },
    };
  }
...
async function getCompaniesReport(startDate, endDate, filters = {}) {
  ...
  if (dimensionFilters.length > 0) {
    request.dimensionFilter = dimensionFilters.length === 1 ? dimensionFilters[0] : { andGroup: { expressions: dimensionFilters } };
  }
...
async function getPagesReport(startDate, endDate, companyFilter) {
  ...
  if (companyFilter) {
    request.dimensionFilter = { filter: { fieldName: 'customUser:name', stringFilter: { value: companyFilter, matchType: 'EXACT' } } };
  }
...
async function getCompanyDetailReport(startDate, endDate, companyName) {
  ...
  const [visitResponse] = await analyticsDataClient.runReport({
    ...
    dimensionFilter: { filter: { fieldName: 'customUser:name', stringFilter: { value: companyName, matchType: 'EXACT' } } },
...
async function getCompaniesByUrlReport(startDate, endDate, urlPath) {
  ...
  const [response] = await analyticsDataClient.runReport({
    ...
    dimensionFilter: { filter: { fieldName: 'pagePath', stringFilter: { value: urlPath, matchType: 'CONTAINS' } } },
    orderBys: [{ metric: { metricName: 'screenPageViews' }, desc: true }],
    limit: 100,
  });
...
async function getSubpagesReport(startDate, endDate, parentPath) {
  ...
  const [response] = await analyticsDataClient.runReport({
    ...
    dimensionFilter: { filter: { fieldName: 'pagePath', stringFilter: { value: parentPath, matchType: 'BEGINS_WITH' } } },
    orderBys: [{ metric: { metricName: 'screenPageViews' }, desc: true }],
    limit: 100,
  });
